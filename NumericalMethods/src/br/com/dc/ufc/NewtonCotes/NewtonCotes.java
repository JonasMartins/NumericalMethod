...
public class NewtonCotes {
	...
	public double fx(double x){
		// return Math.pow(x, 5) - 2*(Math.pow(x, 3)) + 4*x + 5; // função inicial
	...
	public double TrapeziumMethod(){		
		...
		do {
			...
			x = (h/n)/2;
			//limit = 20*(Math.pow(x, 3)) - 12*x; // segunda integral função inicial
			...
			bi = ai+(h/n);
			n++;
			System.out.println("ai after: "+ai);		
			System.out.println("bi after: "+bi);
